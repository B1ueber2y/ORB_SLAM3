...
namespace ORB_SLAM3 {
    class AriaCamera : public GeometricCamera {
    ...
    public:
        AriaCamera() : precision(1e-6) {
            mvParameters.resize(16);
            mnId=nNextId++;
            mnType = CAM_FISHEYE;
        }
        AriaCamera(const std::vector<float> _vParameters) : GeometricCamera(_vParameters), precision(1e-6), mvLappingArea(2,0) ,tvr(nullptr) {
            assert(mvParameters.size() == 16);
            mnId=nNextId++;
            mnType = CAM_FISHEYE;
        }

        AriaCamera(const std::vector<float> _vParameters, const float _precision) : GeometricCamera(_vParameters),
                                                                                        precision(_precision), mvLappingArea(2,0) {
            assert(mvParameters.size() == 16);
            mnId=nNextId++;
            mnType = CAM_FISHEYE;
        }
        AriaCamera(AriaCamera* pAria) : GeometricCamera(pAria->mvParameters), precision(pAria->precision), mvLappingArea(2,0) ,tvr(nullptr) {
            assert(mvParameters.size() == 16);
            mnId=nNextId++;
            mnType = CAM_FISHEYE;
        }
