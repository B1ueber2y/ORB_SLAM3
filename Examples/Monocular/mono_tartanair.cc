...
{  
    if(argc < 5)
    {
        cerr << endl << "Usage: ./mono_tartanair path_to_vocabulary path_to_settings path_to_sequence_folder_1 path_to_times_file_1 (path_to_image_folder_2 path_to_times_file_2 ... path_to_image_folder_N path_to_times_file_N) (trajectory_file_name)" << endl;
        return 1;
    }
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        LoadImages(string(argv[(2*seq)+3]) + "/image_left", string(argv[(2*seq)+4]), vstrImageFilenames[seq], vTimestampsCam[seq]);
        cout << "LOADED!" << endl;
    ...
    // std::vector<MapPoint*> mapPoints = SLAM.GetTrackedMapPoints();
                ...
                vector<string> &vstrImages, vector<double> &vTimeStamps)
{
    ...
    while(!fTimes.eof())
    {
        ...
        if(!s.empty())
        {
            stringstream ss;
            ss << s;
            vstrImages.push_back(strImagePath + "/" + ss.str() + "_left.png");
            double t;
            ss >> t;
