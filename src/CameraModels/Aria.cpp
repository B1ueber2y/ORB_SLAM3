...
//BOOST_CLASS_EXPORT_IMPLEMENT(ORB_SLAM3::AriaCamera)

namespace ORB_SLAM3 {
//BOOST_CLASS_EXPORT_GUID(AriaCamera, "AriaCamera")

    cv::Point2f AriaCamera::project(const cv::Point3f &p3D) {
      // TODO: adapt to aria
    ...
    Eigen::Vector2d AriaCamera::project(const Eigen::Vector3d &v3D) {
      // TODO: adapt to aria
    ...
    Eigen::Vector2f AriaCamera::project(const Eigen::Vector3f &v3D) {
      // TODO: adapt to aria
    ...
    Eigen::Vector2f AriaCamera::projectMat(const cv::Point3f &p3D) {
        cv::Point2f point = this->project(p3D);
        return Eigen::Vector2f(point.x, point.y);
    }

    float AriaCamera::uncertainty2(const Eigen::Matrix<double,2,1> &p2D)
    {
    ...
    Eigen::Vector3f AriaCamera::unprojectEig(const cv::Point2f &p2D) {
        cv::Point3f ray = this->unproject(p2D);
        return Eigen::Vector3f(ray.x, ray.y, ray.z);
    }

    cv::Point3f AriaCamera::unproject(const cv::Point2f &p2D) {
      // TODO: adapt to aria
    ...
    Eigen::Matrix<double, 2, 3> AriaCamera::projectJac(const Eigen::Vector3d &v3D) {
      // adapt to aria
    ...
    bool AriaCamera::ReconstructWithTwoViews(const std::vector<cv::KeyPoint>& vKeys1, const std::vector<cv::KeyPoint>& vKeys2, const std::vector<int> &vMatches12,
    ...
    bool AriaCamera::epipolarConstrain(GeometricCamera* pCamera2, const cv::KeyPoint &kp1, const cv::KeyPoint &kp2,
    ...
    bool AriaCamera::matchAndtriangulate(const cv::KeyPoint& kp1, const cv::KeyPoint& kp2, GeometricCamera* pOther,
    ...
    float AriaCamera::TriangulateMatches(GeometricCamera *pCamera2, const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const Eigen::Matrix3f& R12, const Eigen::Vector3f& t12, const float sigmaLevel, const float unc, Eigen::Vector3f& p3D) {
    ...
    std::ostream & operator<<(std::ostream &os, const AriaCamera &kb) {
        os << kb.mvParameters[0] << " " << kb.mvParameters[1] << " " << kb.mvParameters[2] << " " << kb.mvParameters[3] << " "
           ...
           << kb.mvParameters[8] << " " << kb.mvParameters[9] << " " << kb.mvParameters[10] << " " << kb.mvParameters[11] << " "
           << kb.mvParameters[12] << " " << kb.mvParameters[13] << " " << kb.mvParameters[14] << " " << kb.mvParameters[15];
        return os;
    }

    std::istream & operator>>(std::istream &is, AriaCamera &kb) {
        float nextParam;
    ...
    void AriaCamera::Triangulate(const cv::Point2f &p1, const cv::Point2f &p2, const Eigen::Matrix<float,3,4> &Tcw1,
    ...
    bool AriaCamera::IsEqual(GeometricCamera* pCam)
    {
        if(pCam->GetType() != GeometricCamera::CAM_FISHEYE)
            return false;

        AriaCamera* pKBCam = (AriaCamera*) pCam;
